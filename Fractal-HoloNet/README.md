...
## Контур · Радар рынка (MVP)

Небольшой интерфейс для поиска и квалификации публичных сигналов о спросе на
промышленные реле, контакторы и коммутационную аппаратуру. Он формирует ссылки
на открытые поиски ЕИС, B2B-Center, Фабриканта и агрегатора, помогает разбирать
фрагменты ТЗ и показывает, какие технические параметры необходимо подтвердить
перед предложением российского аналога.

```bash
pip install -r requirements.txt
uvicorn market_service:app --host 0.0.0.0 --port 8080
```

Откройте `http://localhost:8080`. Текущая очередь лидов помечена как **демо**:
она намеренно состоит из синтетических организаций и не утверждает факт чьей-либо
закупки. Для рабочего контура подключите разрешённые API/выгрузки источников и
храните исходный URL, дату публикации и текст документа для ручной верификации.
