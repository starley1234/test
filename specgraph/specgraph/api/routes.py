...
from fastapi import APIRouter, Body, Cookie, Depends, File, HTTPException, Response, UploadFile
...
def _current_ws(db: Session, response: Response, user: User | None, specgraph_ws: str | None):
    from specgraph.retrieval.workspace_store import COOKIE, get_or_create

    row = get_or_create(db, guest_key=specgraph_ws, user=user)
    response.set_cookie(COOKIE, row.guest_key or row.id, httponly=True, samesite="lax", max_age=30 * 24 * 3600)
    return row
...
def get_workspace(
    ...
    specgraph_ws: str | None = Cookie(default=None),
):
    from specgraph.retrieval.ask import workspace as dump

    row = _current_ws(db, response, user, specgraph_ws)
    data = dump(db, row.document_ids or [])
    data["id"] = row.id
    data["include_graph"] = bool(row.include_graph)
    return data
...
def patch_workspace(
    ...
    from specgraph.retrieval.workspace_store import attach_documents, set_include

    row = _current_ws(db, response, user, specgraph_ws)
    if "include_graph" in body:
        set_include(db, row, bool(body["include_graph"]))
    if body.get("document_ids"):
        attach_documents(db, row, [int(x) for x in body["document_ids"]])
    return {"id": row.id, "include_graph": row.include_graph, "document_ids": row.document_ids}
...
def ask_workspace(
    ...
    from specgraph.retrieval.workspace_store import log_event

    row = _current_ws(db, response, user, specgraph_ws)
    q = str(body.get("question") or body.get("q") or "").strip()
    if not q:
        raise HTTPException(400, "нужен question")
    include = body.get("include_graph")
    if include is None:
        include = row.include_graph
    out = ask(db, q, document_ids=list(row.document_ids or []), include_graph=bool(include))
    log_event(db, row, "ask", {"q": q[:200], "route": out.get("route")})
    return out
