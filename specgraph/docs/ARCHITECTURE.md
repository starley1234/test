...
Карта хранения и сборки контекста: `docs/CONTEXT.md`.

Кратко: граф (изделия + требования + рёбра) — основа. Чанки только у файлов **без** карточек. Картинки — файлы в `data/media/`, `GET /illustrations/{id}`. Сборщик: `mode=graph|hybrid|chunks`.
